...
def  get_fitness1(aim,pop,cpu_base,mem_base,worker,ps):
    ...
    if mode == 0:
        ...
        if not aim2:
            ...
            if pop_new.size==0 and not change_number:
                ...
                if aim1_cpu < 0.45 * res_config0[aim1]['cpu_high']:
                    aim1_cpu = math.ceil(0.45 * res_config0[aim1]['cpu_high'])
                if aim1_mem < 0.9 * res_config0[aim1]['memory_base']:
                    aim1_mem = math.ceil(0.9 * res_config0[aim1]['memory_base'])
                method2 = {'type': 2, 'cpu': aim1_cpu, 'mem': aim1_mem}
                assign_config[aim1] = method2
            elif change_number and pop_new.size>0:
                fitness = get_fitness4(aim1, pop_new,res_config0[aim1]['cpu_high'],
        ...
        for _ in range(N_GENERATIONS):
            ...
            pop = np.array(pop)
            # fitness = get_fitness3()
            fitness = get_fitness4(aim1,pop,res_config0[aim1]['cpu_high'],res_config0[aim1]['memory_base'],res_config0[aim1]['cpu_source'],res_config0[aim1]['mem_source'],res_config0[aim1]['worker'],res_config0[aim1]['mini'])
            pop = select(pop, fitness)  # 选择生成新的种群
            if len(pop_total)>= 2**(2*DNA_SIZE)-1:
                break
        print(len(pop_total))
        ...
        if pop_new.size==0 and not change_number:
            ...
            fitness = get_fitness4(aim1,pop_new,res_config0[aim1]['cpu_high'],res_config0[aim1]['memory_base'],res_config0[aim1]['cpu_source'],
                                   res_config0[aim1]['mem_source'],res_config0[aim1]['worker'],res_config0[aim1]['mini'])
            max_fitness_index = np.argmax(fitness)
            print("max_fitness:", fitness[max_fitness_index])
            cpu_mod, mem_mod = translateDNA2(pop_new)
            ...
            if count2 < 3:
                ...
                if job.memory_allocate > res_config['memory_base']:
                    ...
                    if math.ceil(job.memory_allocate * 0.92) >= res_config['memory_base']:
                        ...
                        job.assignment_resource(job.cpu_allocate, math.ceil(job.memory_allocate * 0.9))
                    else:
...
def Submit_job(tasks,lock,v1,jobs):
    ...
    while True:
        ...
        if tasks['start']==False:
            break
        if global_count > 3 and global_count % 7 != 0 and global_count%10 != 0:
            lock.acquire()
            counts = tasks['count']
            ...
            if tasks['next'] and counts < tasks['size']:
                ...
                if catch_ps > 0 and catch_worker > 0:
                    ...
                    res_config = load_config(save_res_path)
                    if job.cpu_allocate > math.ceil(0.45*res_config['cpu_high']):
                        if math.ceil(job.cpu_allocate * 0.75) >= math.ceil(0.45*res_config['cpu_high']):
                            save_job_change_resource(job.name, math.ceil(job.cpu_allocate * 0.75),
                                                     job.memory_allocate)
                        else:
                            save_job_change_resource(job.name, math.ceil(0.45*res_config['cpu_high']), job.memory_allocate)
                        # job.assignment_resource(res_config['cpu_high'], job.memory_allocate)
                        print("modulate the cpu!!")
                    else:
                        if job.memory_allocate > res_config['memory_base']:
                ...
                if catch_ps < job.ps_replicas or catch_worker < job.worker_replicas:
                    ...
                    lock.release()
            global_count+=1
            time.sleep(sleep_last_length*1.125)
        if global_count % 10 == 0 or global_count<=3:
                ...
                #         time.sleep(30)
                    ...
                    if tasks['count'] < tasks['size'] or tasks['buffercount'] < max_buffer_size:
                        ...
                        if ress_lists[0] == '400':
                            ...
                            cpu_base = math.ceil(1.15 * cpu_predict)
                            mem_predict = float(ress_lists[-1])
                            mem_base = math.ceil(1.48 * mem_predict)
                            res_to_server = '1'
