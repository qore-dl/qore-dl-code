...
class Node_mess(multiprocessing.Process):
    ...
    def run(self):
        ...
        tmp_cre = tasks['creation']
        tmp_cre.append(match_timestamp(response['items'][0]['metadata']['creationTimestamp']))
        tasks['creation'] = tmp_cre
        ...
        while True:
            if tasks['start'] == False:
                response_end = catch_message(self.url)
                tmp_end = tasks['endtime']
                tmp_end.append(match_timestamp(response_end['items'][0]['metadata']['creationTimestamp']))
                tasks['endtime'] = tmp_end
                break
            response = catch_message(self.url)
...
def Submit_job(tasks,lock,v1):
    while True:
        ...
        if counts >= tasks['size']:
            ...
            worker_r = random.randint(2,5)
            batch = random.randint(128,800)
            print(tasks)
            if template_id == 1:
