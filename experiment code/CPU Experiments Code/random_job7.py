...
job_basic0 = reload_jobs("res-292-292",-1)
...
def  get_fitness1(aim,pop,cpu_now,mem_now,cpu_base,mem_base,worker,ps):
    ...
    # if cpu_pre > cpu_pre_limit:
    #     cpu_pre = cpu_pre_limit
    # if mem_pre > mem_pre_limit:
    #     mem_pre = mem_pre_limit
...
def assign_jobs(tasks,rfr,lock):
    ...
    # cpu_pre = cpu_now * cpu_alpha
    # # mem_pre = mem_base*mem_alpha
    # cpu_pre_limit = XBOUND[-1] * 1.12 * cpu_base
    # mem_pre = mem_now * mem_alpha
    # mem_pre_limit = YBOUND[-1] * 1.15 * mem_base
    global pop_total
    #选择标准有两个：
    '''
    ...
    self.node_cpu['k8s-master'] = 32000
        ...
        self.node_cpu['k8s-worker1'] = 16000
        ...
        if aim_time[-1] <= 0:
            ...
            if relise > 0.4:
                ...
                if len(aim_time) == 1 and len(aim[aim_time[-1]]) == 1:
                    ...
                    # aim1_job = reload_jobs(aim1,-1)
                    # res_config0[aim1]['cpu_source'],res_config0[aim1]['mem_source'],res_config0[aim1]['cpu_high'], res_config0[aim1]['memory_base']
                    ...
                    for atkk in range(ttll - 1, -1, -1):
                        for using_aim in aim[aim_time[atkk]]:
                            ...
                            if res_config0[using_aim]['worker'] > 8 and res_config0[using_aim]['cpu_source'] >= cpu_pre_limit and res_config0[using_aim]['mem_source'] >= mem_pre_limit:
                                continue
                            else:
                                aim1 = using_aim
                                tmp_rank = atkk
                ...
                if len(aim_time) == 1 and len(aim[aim_time[0]]) == 1:
                    ...
                    # cpu_pre_limit = math.ceil(XBOUND[-1] * 1.5 * res_config0[aim1]['cpu_high'])
                    # mem_pre_limit = math.ceil(YBOUND[-1] * 1.92 * res_config0[aim1]['memory_base'])
                    # if res_config0[aim1]['worker'] > 8 and res_config0[aim1]['cpu_source'] >= cpu_pre_limit and \
                    #         res_config0[aim1]['mem_source'] >= mem_pre_limit:
                    #     aim1 = ''
                    aim2 = ''
                    ...
                    # ttll = len(aim_time)
                    # tmp_rank = -1
                    # for atkk in range(0, ttll):
                    #     for using_aim in aim[atkk]:
                    #         cpu_pre_limit = math.ceil(XBOUND[-1] * 1.12 * res_config0[using_aim]['cpu_high'])
                    #         mem_pre_limit = math.ceil(YBOUND[-1] * 1.15 * res_config0[using_aim]['memory_base'])
                    ...
                    aim_mingan = {}
                    for j in aim_time_po:
                        # cpu_pre_limit = math.ceil(XBOUND[-1] * 1.12 * res_config0[j]['cpu_high'])
                        # mem_pre_limit = math.ceil(YBOUND[-1] * 1.15 * res_config0[j]['memory_base'])
                        # if res_config0[j]['worker'] > 8 and res_config0[j]['cpu_source'] >= cpu_pre_limit and \
                        #         res_config0[j]['mem_source'] >= mem_pre_limit:
                        #     continue
                        reload_tmp = reload_jobs(j, -1)
    ...
    if mode == 0:
        ...
        if aim1:
            ...
            if pop_new.size == 0 and not change_number:
                ...
                if res_config0[aim1]['worker'] > 8 and res_config0[aim1]['cpu_source'] >= cpu_pre_limit and \
                    ...
                    # assign_config[aim1] = method2
                    # fitness = get_fitness1(aim1, pop_new, res_config0[aim1]['cpu_source'], res_config0[aim1]['mem_source'],
                    #                        res_config0[aim1]['cpu_high'], res_config0[aim1]['memory_base'],
                    #                        res_config0[aim1]['worker'], res_config0[aim1]['ps'])
                    # max_fitness_index = np.argmax(fitness)
                    # print("max_fitness:", fitness[max_fitness_index])
        ...
        if not aim2:
            ...
            for _ in range(N_GENERATIONS):
                ...
                #                        res_config0[aim1]['worker'], res_config0[aim1]['ps'])
                fitness = get_fitness1(aim1, pop,res_config0[aim1]['cpu_source'],res_config0[aim1]['mem_source'],res_config0[aim1]['cpu_high'], res_config0[aim1]['memory_base'],
                                       res_config0[aim1]['worker'], res_config0[aim1]['ps'])
                pop = select(pop, fitness)  # 选择生成新的种群
                if len(pop_total) >= 2 ** (2 * DNA_SIZE) - 1:
                    break
            ...
            if pop_new.size==0 and not change_number:
                ...
                cpu_pre_limit = math.ceil(XBOUND[-1] * 1.478 * res_config0[aim1]['cpu_high'])
                mem_pre_limit = math.ceil(YBOUND[-1] * 1.682 * res_config0[aim1]['memory_base'])
                if res_config0[aim1]['cpu_source'] >= cpu_pre_limit and res_config0[aim1]['mem_source'] >= mem_pre_limit:
                    assign_config[aim1] = {}
                # fitness = get_fitness1(aim1, pop_new,res_config0[aim1]['cpu_high'], res_config0[aim1]['memory_base'],
                #                        res_config0[aim1]['worker'], res_config0[aim1]['ps'])
                else:
        ...
        if not aim2:
            ...
            if pop_new.size==0 and not change_number:
                ...
                fitness = get_fitness4(aim1, pop_new, res_config0[aim1]['cpu_high'],
                                       ...
                                       res_config0[aim1]['mem_source'], res_config0[aim1]['worker'],res_config0[aim1]['mini'])
                max_fitness_index = np.argmax(fitness)
                print("max_fitness:", fitness[max_fitness_index])
        ...
        if not aim2:
            ...
            for _ in range(N_GENERATIONS):
                ...
                for item in pop_tmp:
                    for item2 in item:
                        pop.append(item2)
                pop = np.array(pop)
                # fitness = get_fitness3(aim1, pop, res_config0[aim1]['cpu_high'],
                #                        res_config0[aim1]['memory_base'])
                fitness = get_fitness3(aim1, pop,res_config0[aim1]['cpu_source'],res_config0[aim1]['mem_source'],res_config0[aim1]['cpu_high'],
                                       res_config0[aim1]['memory_base'])
                pop = select(pop, fitness)  # 选择生成新的种群
                if len(pop_total) >= 2 ** (2 * DNA_SIZE) - 1:
                    break
            ...
            if pop_new.size==0 and not change_number:
                ...
                if res_config0[aim1][
                    ...
                    # fitness = get_fitness3(aim1, pop_new,res_config0[aim1]['cpu_high'],
                    #                        res_config0[aim1]['memory_base'])
                    fitness = get_fitness3(aim1, pop_new, res_config0[aim1]['cpu_source'],
                                           res_config0[aim1]['mem_source'], res_config0[aim1]['cpu_high'],
                                           res_config0[aim1]['memory_base'])
...
def make_label_to_nodes(tasks2,base_job_name):
    ...
    base_job = reload_jobs(base_job_name,-1)
...
def generate_item(response,measurement):
    ...
    # node_memory['k8s-master'] = float(251 * 1024 - 9400)
    # node_memory['k8s-worker0'] = float(94 * 1024 - 1000)
    # node_memory['k8s-worker2'] = float(94 * 1024 - 3200)
    # node_memory['k8s-worker1'] = float(125 * 1024 - 1500)
    # node_memory['k8s-worker3'] = float(94 * 1024 - 1100)
    # node_memory['k8s-worker4'] = float(188 * 1024 - 1100)
    # node_memory['k8s-worker5'] = float(125 * 1024 - 4200)
    # node_memory['k8s-worker6'] = float(94 * 1024 - 2000)
    # node_memory['k8s-worker7'] = float(94 * 1024 - 1250)
    # node_memory['k8s-worker8'] = float(62 * 1024 - 1250)
    # node_memory['k8s-worker9'] = float(62 * 1024 - 1200)
    # node_memory['k8s-worker10'] = float(62 * 1024 - 1200)
    # node_memory['k8s-worker11'] = float(94 * 1024 - 1200)
    # # node_memory['k8s-worker12'] = float(62 * 1024 - 2000)
    # # node_memory['k8s-worker13'] = float(62 * 1024 - 2000)
    # node_memory['k8s-worker12'] = float(94 * 1024 - 2000)
    # node_memory['k8s-worker13'] = float(94 * 1024 - 2000)
    # node_memory['k8s-worker14'] = float(62 * 1024 - 2000)
    # # node_memory['k8s-worker15'] = float(62 * 1024 - 2000)
    # node_memory['k8s-worker15'] = float(125 * 1024 - 2000)
    # # node_memory['k8s-worker16'] = float(62 * 1024 - 2000)
    # node_memory['k8s-worker16'] = float(94 * 1024 - 2000)
    # # node_memory['k8s-worker17'] = float(94 * 1024 - 2000)
    # node_memory['k8s-worker17'] = float(62 * 1024 - 2000)
    # # node_memory['k8s-worker18'] = float(62 * 1024 - 2000)
...
class Node_mess(multiprocessing.Process):
    def __init__(self,url,args,tasks,v1):
        ...
        # self.node_cpu['k8s-worker15'] = 32000 - 200
        ...
        # self.node_cpu['k8s-worker5'] = 32000 - 150
        # self.node_cpu['k8s-worker6'] = 24000 - 150
        # self.node_cpu['k8s-worker7'] = 24000 - 150
        ...
        # self.node_cpu['k8s-worker12'] = 24000 - 150
        # self.node_cpu['k8s-worker13'] = 24000 - 150
        # self.node_cpu['k8s-worker14'] = 16000 - 150
        # self.node_cpu['k8s-worker15'] = 32000 - 150
        # self.node_cpu['k8s-worker16'] = 24000 - 150
        # self.node_cpu['k8s-worker17'] = 16000 - 150
        # # node_cpu['k8s-worker18'] = 16000 - 150
        ...
        # self.node_memory['k8s-worker5'] = float(125 * 1024 - 2200)
        # self.node_memory['k8s-worker6'] = float(94 * 1024 - 2000)
        # self.node_memory['k8s-worker7'] = float(94 * 1024 - 2000)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.client.write_points(generate_item(response, self.measurement), 's', database=self.database)
                # if len(self.time_mess['creation']) % 6 == 0:
                #     print("finish 6 times of checking node message!!!")
                #     print("let us go ahead!!!")
                if len(self.time_mess['creation']) % 45 == 0 and len(self.time_mess['creation']) > 0:
                    ...
                    data_frame2.to_csv(self.save_path + '/' + 'node5_cpu.csv', mode='a+', index=False, sep=',')
                    data_frame3 = pd.DataFrame(self.memory_mess)
                    data_frame3.to_csv(self.save_path + '/' + 'node5_memory.csv', mode='a+', index=False, sep=',')
                    data_frame4 = pd.DataFrame(self.cpu_per)
                    data_frame4.to_csv(self.save_path + '/' + 'node5_cpu_per.csv', mode='a+', index=False, sep=',')
                    data_frame5 = pd.DataFrame(self.memory_per)
                    data_frame5.to_csv(self.save_path + '/' + 'node5_memory_per.csv', mode='a+', index=False, sep=',')
                    f1 = open('/tfdata/nodedata/node5.json', 'r', encoding='utf-8')
                    res = f1.read()
                    a = json.loads(res)
                    f1.close()
                    node_layout = {}
                    ...
                    f2 = open('/tfdata/nodedata/node5.json', 'w', encoding='utf-8')
...
def catch_node_step_msg(jobs,job_name,tasks,lock,batch,flops,params,mode,task2):
    ...
    while True:
            ...
            if count2 < 3:
                ...
                if count23 % 5 == 0:
                    ...
                    if job.memory_allocate > 1.15 * res_config['memory_base']:
                        ...
                        # elif math.ceil(job.memory_allocate * 0.98) >= 1.11*res_config['memory_base']:
                        #     save_job_change_resource(job.name, job.cpu_allocate, math.ceil(job.memory_allocate * 0.98))
                        #     job.assignment_resource(job.cpu_allocate, math.floor(job.memory_allocate * 0.98))
                        else:
            ...
            #     time.sleep(3.58)
                ...
                if count23 % 5 == 0:
                    if job.ps_replicas > 1:
                        ...
                        tmp_replicas = job.worker_replicas
                        print("reduce worker number!!")
                        # bili = math.floor(job.worker_replicas / job.ps_replicas)
                        # aim_tmp_replicas =
                        # aim_step = step_influx_client.query("select training_step from " + measure_t + " order by desc limit 1")
                        aim_steps = step_influx_client.query(
                            "select training_step from " + measure_t + " order by desc limit 1")
                ...
                if count23 % 5 == 0:
                    ...
                    # time.sleep(17.9)
                time.sleep(3.58)
        # elif run_result_dict['Running'] == 1 and 'Pending' not in pod_status and 'Succeeded' not in pod_status and 'Failed' not in pod_status:
        #     count111 = 0
        #     count = 0
        #
        #     save_res_path = '/tfdata/k8snfs/%s/%s_res.json' % (job.name, job.name)
        ...
        #         print("deal with apply operation!!")
        #
        #         try:
        #             command = 'kubectl apply -f /tfdata/tfcnn/expjob/' + job.name + '.yaml'
        #             os.system(command)
        #         except Exception as e001:
        #             print(e001)
        #             try:
        #                 job.v1.create_namespace(job.name)
        #                 command = 'kubectl apply -f /tfdata/tfcnn/expjob/' + job.name + '.yaml'
        #                 os.system(command)
        #             except Exception as e0022:
        #                 print(e0022)
        #         time.sleep(5.6)
        #         # try:
        #         #     command = 'kubectl apply -f /tfdata/tfcnn/expjob/' + job.name + '.yaml'
        ...
        #         # print(aim_step)
        #         # # aim_step = math.ceil((aim_step * tmp_replicas) / (job.worker_replicas - 1))
        #         # kk_ps_replicas = job.ps_replicas
        #         # kk_wk_replicas = job.worker_replicas
        #         # if kk_ps_replicas <1:
        #         #     kk_ps_replicas = 1
        #         # if kk_wk_replicas < 1:
        #         #     kk_wk_replicas = 1
        #         # save_job_change_layout(job.name,kk_ps_replicas,kk_wk_replicas, aim_step, mode=1)
        #         # if int(job.mod) != 4:
        #         #     job.set_mod(4)
        #         #     save_mode_change(4, job.name, float(time.time()))
        #         # # lock.acquire()
        ...
        #         #         tasks['retry'] = tmp_retry_job
        #         #         time1 = time.time()
        #         #         job.retry_tf(job.cpu_allocate, job.memory_allocate, aim_step, kk_wk_replicas,
        #         #                      kk_ps_replicas)
        #         #         time2 = time.time()
        #         #         tmp_retry_job = tasks['retry']
        #         #         tmp_retry_job.remove(job.name)
        ...
        #
        #         count2 += 1
        #         print("apply this job %s success!!" % job.name)
        #         time.sleep(7.35)
        # elif not run_result_dict_c:
        #     print("run_result_dict is null")
        #     ceshi_tmp_ns = tasks['ns']
        #     if job.name not in ceshi_tmp_ns:
        #         if count111 <= 18:
        #             count111+=1
        #             time.sleep(3.8)
        #             print("%s sleep a little" % job.name)
        #             continue
        #         else:
        #             print("%s : this is a missing mistake!!!" % job.name)
        #             return
        #     else:
        #         count111 = 0
        #         count += 1
        #         if count >= 12:
        #             print("%s deal with apply operation!" % job.name)
        #             try:
        #                 command = 'kubectl apply -f /tfdata/tfcnn/expjob/' + job.name + '.yaml'
        #                 os.system(command)
        #             except Exception as e001:
        ...
        #             time.sleep(5.6)
        #             count = 0
        #             print("%s done apply operation!" % job.name)
        #             continue
        #
        #             # jieshu = True
        ...
        #             # save_res_path = '/tfdata/k8snfs/%s/%s_res.json' % (job.name, job.name)
        #             # res_job_config = load_config(save_res_path)
        #             # res_job_config['errtime'] = time.time() - 36
        #             # save_config(res_job_config, save_res_path)
        #             # job.set_mod(7)
        #             # save_mode_change(7, job.name, float(res_job_config['errtime']))
        #             # aim_steps = step_influx_client.query(
        #             #     "select training_step from " + measure_t + " order by desc limit 1")
        #             # aim_key = aim_steps.keys()
        #             # result_inter = aim_steps[aim_key[0]]
        #             # result_items = list(result_inter)
        #             # aim_step = int(result_items[0]['training_step']) * job.worker_replicas
        #             # print(aim_step)
        #             # save_job_change_layout(job.name, 1, 1, aim_step, mode=1)
        #             # job.set_mod(3)
        #             # save_mode_change(3, job.name, float(res_job_config['errtime']))
        #             # lock.acquire()
        #             # try:
        #             #     try:
        #             #         command = 'kubectl delete -f /tfdata/tfcnn/expjob/' + job.name + '.yaml'
        #             #         os.system(command)
        #             #     except Exception as e000000:
        #             #         print(e000000)
        #             #     deletehelp2(job.name, v1)
        #             #     # v1.delete_namespace(job.name)
        #             #     ns_tmp = tasks['ns']
        ...
        #             #     lock.release()
        #             #     print(eess)
        #             # print("%s finish go to next because of create problem" % job.name)
        #
        #             # lock.acquire()
        #             # try:
        #             #     try:
        ...
        #             # return
        #         else:
        #             print("%s sleep a little with %d count" % (job.name, count111))
        #             time.sleep(5.2)
        #             continue
        else:
    ...
    while True:
        ...
        if job.name in tmp_retrys and job.name in solution_keys:
            ...
            #     time.sleep(6)
                ...
                if count22 < 4:
                    ...
                    if count223 % 5 == 0:
                        ...
                        #     #  "cpu_high": 15780,
                        #     #     "memory_base": 32113,
                        #     if math.ceil(job.memory_allocate * 0.92) >= res_config['memory_base']:
                        #         save_job_change_resource(job.name, job.cpu_allocate, math.ceil(job.memory_allocate * 0.93))
                        #         job.assignment_resource(job.cpu_allocate, math.ceil(job.memory_allocate * 0.93))
                        #     if math.ceil(job.memory_allocate * 0.92) >= res_config['memory_base']:
                        #         save_job_change_resource(job.name, job.cpu_allocate, math.ceil(job.memory_allocate * 0.93))
                        #         job.assignment_resource(job.cpu_allocate, math.ceil(job.memory_allocate * 0.93))
                        #     else:
                        ...
                        if job.memory_allocate > 1.15 * res_config['memory_base']:
                            ...
                            # elif math.ceil(job.memory_allocate * 0.98) >= 1.15 * res_config['memory_base']:
                            #     save_job_change_resource(job.name, job.cpu_allocate, math.ceil(job.memory_allocate * 0.98))
                            #     job.assignment_resource(job.cpu_allocate, math.ceil(job.memory_allocate * 0.98))
                            else:
                                save_job_change_resource(job.name, job.cpu_allocate,
                ...
                # time.sleep(17.9)
                    ...
                    if count223 % 5 == 0:
                        if count22 < 6:
                            kpk = random.randint(1, 4) / 100
                            # #                                         # save_job_change_resource(job.name, math.ceil(job.cpu_allocate*0.9), math.ceil(job.memory_allocate*(1+kpk)))
                            # #                     save_job_change_resource(job.name, math.ceil(job.cpu_allocate*0.95), math.ceil(job.memory_allocate*(1+kpk)))
                            tmp_reuse_mem = math.ceil(job.memory_allocate * (1 + kpk))
                        ...
                        if count22 >= 7:
                            ...
                            # time.sleep(17.5)
                    time.sleep(4)
            # elif run_result_dict2['Running'] == 1 and 'Pending' not in pod_status2 and 'Succeeded' not in pod_status2 and 'Failed' not in pod_status2:
            #             #     count111 = 0
            #             #     count11 = 0
            #             #     if countt00 < 13:
            #             #         countt00 += 1
            ...
            #             #         # job.write_retry(mode=0)
            #             #         count22 += 1
            #             #         print("apply this job %s successfully!!" % job.name)
            #             #         time.sleep(7.35)
            # elif not run_result_dict_c2:
            #     print("result dict c2 is null!!!")
            #     # ceshi_tmp_ns = tasks['ns']
            #     ceshi_tmp_ns = tasks['ns']
            #     if job.name not in ceshi_tmp_ns:
            #         if count111 <= 18:
...
def Submit_job(tasks,lock,v1,jobs,task2):
    ...
    while True:
        ...
        if global_count > 3 and global_count % 11 != 0 and global_count%9 != 0 and global_count%45 > 1:
            lock.acquire()
            counts = tasks['count']
            ...
            if tasks['next'] and counts < tasks['size']:
                ...
                if catch_ps > 0 and catch_worker > 0:
                    ...
                    while True:
                        ...
                        if not task2['label']:
                            # tmp_node_ranks = task2['rank']
                            # tmp_node_cpu_cond = task2['cpu']
                            # tmp_node_mem_cond = task2['mem']
                ...
                selected_res_config_path = '/tfdata/k8snfs/%s/%s_res.json' % (selected_job_name, selected_job_name)
        ...
        if global_count % 9 == 0 or global_count<=3 or global_count%45==1:
            ...
            for _ in range(10):
                ...
                bufer_count = tasks['buffercount']
                lock.release()
                if (counts >= tasks['size']) and (bufer_count >= max_buffer_size) or now_create>=76:
                ...
                #         time.sleep(30)
                    ...
                    if tasks['reload'] == 0:
                        ...
                        if template_id == 1:
                            ...
                            dict12 = {'batch': batch, 'channel1': channel1, 'channel2': channel2, 'channel3': channel3,
                                    'channel4': channel4,
                        ...
                        if template_id == 1:
                            ...
                            dict12 = {'batch': job.batch_size, 'BC': job.BC, 'k': job.k, 'L': job.L, 'num_classes': 10}
                    ...
                    if tasks['count'] < tasks['size'] or tasks['buffercount'] < max_buffer_size:
                        ...
                        if ress_lists[0] == '400':
                            ...
                            if job.template_id != 4:
                                mem_base = math.ceil(1.657 * mem_predict)
                            else:
                                mem_base = math.ceil(1.867 * mem_predict)
                            res_to_server = '1'
                        ...
                        save_res_path = '/tfdata/k8snfs/%s/%s_res.json' % (job.name, job.name)
                        # save_config(job_config, save_job_path)
                        save_mod_path = '/tfdata/k8snfs/%s/%s_mod.json' % (job.name,job.name)
                        ...
                        if tasks['count'] < tasks['size'] and tasks['buffercount'] == 0:
                            ...
                            #
                            # elif cpu_value < 0.7:
                            #     job.worker_replicas = random.randint(2, 4)
                            #
                            # else:
                            #     job.worker_replicas = random.randint(1, 2)
                            ...
                            tmp_ps_replicas = job.ps_replicas
                            tmp_worker_replicas = job.worker_replicas
                            selected_res_config_path = '/tfdata/k8snfs/%s/%s_res.json' % (
                            ...
                            using_cpu_base = selected_res_config2['cpu_high']
                            using_mem_base = selected_res_config2['memory_base']
                            using_min_batch = predict_min_first(rfr=rfr, cpu_alpha=job.cpu_allocate / using_cpu_base,
...
def save_job_change_layout(job_name,ps_n,worker_n,training_step,mode=0):
    ...
    # save_res_path = '/tfdata/k8snfs/%s/%s_res.json' % (job_name, job_name)
    # job_res_config = load_config(save_res_path)
    save_change_path = '/tfdata/k8snfs/%s/%s_pw.json' % (job_name, job_name)
    try:
...
def save_job_change_resource(job_name,cpu_allocate,mem_allocate):
    ...
    # job_res_config = load_config(save_res_path)
    save_change_path = '/tfdata/k8snfs/%s/%s_change.json' % (job_name, job_name)
    change_dir = {}
    # save_res_path = '/tfdata/k8snfs/%s/%s_res.json' % (job.name, job.name)
    # save_config(job_config, save_job_path)
